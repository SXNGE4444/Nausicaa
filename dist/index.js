...
var messages = mysqlTable("messages", {
  ...
  // gmail, whatsapp, linkedin, slack, discord, etc
  ...
  isLead: int("isLead").default(0).notNull(),
  // 0 or 1 for boolean
...
var toolIntegrations = mysqlTable("toolIntegrations", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id),
  toolName: varchar("toolName", { length: 128 }).notNull(),
  category: varchar("category", { length: 64 }).notNull(),
  // email, communication, project-mgmt, automation, etc
...
var automations = mysqlTable("automations", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id),
...
var agentConversations = mysqlTable("agentConversations", {
  id: int("id").autoincrement().primaryKey(),
...
var activityFeed = mysqlTable("activityFeed", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id),
  action: varchar("action", { length: 128 }).notNull(),
  // message_synced, automation_triggered, tool_connected, etc
...
async function getMessagesByUserId(userId, limit = 20) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(messages).where(eq(messages.userId, userId)).orderBy(desc(messages.createdAt)).limit(limit);
}
async function getToolIntegrationsByUserId(userId) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(toolIntegrations).where(eq(toolIntegrations.userId, userId));
}
async function getAutomationsByUserId(userId) {
  const db = await getDb();
  if (!db) return [];
...
async function getAgentConversation(agentId, userId) {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select().from(agentConversations).where(
    and(
...
async function createOrUpdateAgentConversation(agentId, userId, messages2) {
  const db = await getDb();
  if (!db) return null;
  const existing = await getAgentConversation(agentId, userId);
  if (existing) {
    return db.update(agentConversations).set({ messages: messages2, updatedAt: /* @__PURE__ */ new Date() }).where(eq(agentConversations.id, existing.id));
  } else {
...
async function getActivityFeedByUserId(userId, limit = 20) {
  const db = await getDb();
  if (!db) return [];
...
async function onAutomationTriggered(automationName, status) {
  return notifyOwner2({
...
async function onSystemAlert(title, message) {
  return notifyOwner2({
    title,
    content: message,
    type: "system"
...
async function handleAutomationExecution(automationId, automationName, success, errorMessage) {
  if (success) {
...
var appRouter = router({
  ...
  // Messages / Inbox
  messages: router({
    list: protectedProcedure.input(z2.object({ limit: z2.number().default(20) })).query(async ({ input, ctx }) => {
  ...
  agents: router({
    ...
    chat: protectedProcedure.input(
      ...
      try {
        ...
        const assistantMessage = typeof response.choices[0]?.message?.content === "string" ? response.choices[0].message.content : "No response";
        conversationMessages.push({
          role: "assistant",
  ...
  notifications: router({
    ...
    create: protectedProcedure.input(
      z2.object({
        title: z2.string(),
        content: z2.string(),
        type: z2.enum(["automation", "agent", "integration", "system"])
      })
    ).mutation(async ({ input, ctx }) => {
  ...
  automation: router({
    trigger: protectedProcedure.input(z2.object({ automationId: z2.number() })).mutation(async ({ input, ctx }) => {
      try {
        const automations2 = await getAutomationsByUserId(ctx.user.id);
        const automation = automations2?.find((a) => a.id === input.automationId);
        if (!automation) {
